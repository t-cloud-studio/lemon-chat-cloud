package com.tcloud.redis.config;
...
public class RedisClient {
     ...
     * @param key rediskey
     */
    public Integer getIntSetDefault(String key, Integer defaultVar) {
        String v = this.get(key);
        if (CharSequenceUtil.isBlank(v)) {
            this.set(key, defaultVar);
            return defaultVar;
        }
        return Integer.parseInt(v);
    }
    ...
    public <T> T get(String key, TypeReference<T> reference) {
        ...
        return JSON.parseObject(v, reference);
    }
